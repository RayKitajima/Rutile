...
{{#PrimaryKeySearches}}{{#search_types}}
	'{{segment}}/{{entity}}.{{primary_key}}({{search_type}})' : $.{{segment}}_{{entity}}_{{primary_key}}_{{search_type}}_SearchForm,
{{/search_types}}{{/PrimaryKeySearches}}
{{#Fields}}{{#search_types}}
	'{{segment}}/{{entity}}.{{fieldName}}({{search_type}})' : $.{{segment}}_{{entity}}_{{fieldName}}_{{search_type}}_SearchForm,
{{/search_types}}{{/Fields}}
{{#Fields_joined}}{{#search_types_joined}}
	'{{segment_joined}}/{{entity_joined}}.{{fieldName_joined}}({{search_type_joined}})' : $.{{segment_joined}}_{{entity_joined}}_{{fieldName_joined}}_{{search_type_joined}}_SearchForm,
{{/search_types_joined}}{{/Fields_joined}}
{{#Fields_collected}}{{#search_types_collected}}
	'{{segment_collected}}/{{entity_collected}}.{{fieldName_collected}}({{search_type_collected}})' : $.{{segment_collected}}_{{entity_collected}}_{{fieldName_collected}}_{{search_type_collected}}_SearchForm,
...
formGroup.setSubmitAction({
	...
	'handler' : function(){
		var controller = Alloy.createController('KitchenSink/{{segment}}/{{entity}}/ListReusable');
...
// delgate object should have listenProductImageSelection()
