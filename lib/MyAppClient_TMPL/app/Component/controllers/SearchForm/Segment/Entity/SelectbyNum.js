...
var addItem = function(){
	var item = Alloy.createController('Framework/SearchFormElementNum');
	item.setIndex(items.length);
	item.setClearButtonHandler(function(){
		var min_max = item.getValue();
...
// for internal program, adding Framework/SearchFormElementNum instance without ui
exports.addItem = function(item){
	items.push(item);
};

// for internal program, removing Framework/SearchFormElementNum instance without ui
...
exports.getTextExpression = function(){
	...
	if( texts.length > 0 ){
		...
		textExp += ' (' + L('FrameworkTextExpSearchTypeNum') + ')';
		textExp += ' : ';
		textExp += texts.join(', ');
		if( texts.length > 0 ){
			textExp += ' (' + logic + ')';
