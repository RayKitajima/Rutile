...
var addItem = function(){
	var item = Alloy.createController('Framework/SearchFormElementLike');
...
// for internal program, adding Framework/SearchFormElementLike instance without ui
exports.addItem = function(item){
	items.push(item);
};

// for internal program, removing Framework/SearchFormElementLike instance without ui
...
exports.getTextExpression = function(){
	...
	if( texts.length > 0 ){
		...
		textExp += ' (' + L('FrameworkTextExpSearchTypeLike') + ')';
		textExp += ' : ';
		textExp += texts.join(', ');
		if( texts.length > 0 ){
			textExp += ' (' + logic + ')';
