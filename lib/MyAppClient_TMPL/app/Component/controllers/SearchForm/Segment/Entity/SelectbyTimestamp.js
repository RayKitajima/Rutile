...
var addItem = function(){
	var item = Alloy.createController('Framework/SearchFormElementTimestamp');
	item.setIndex(items.length);
	item.setClearButtonHandler(function(){
		var from_to = item.getValue();
...
// for internal program, adding Framework/SearchFormElementTimestamp instance without ui
exports.addItem = function(item){
	items.push(item);
};

// for internal program, removing Framework/SearchFormElementTimestamp instance without ui
...
exports.getTextExpression = function(){
	...
	if( texts.length > 0 ){
		...
		textExp += ' (' + L('FrameworkTextExpSearchTypeTimestamp') + ')';
		textExp += ' : ';
		textExp += texts.join(', ');
		if( texts.length > 0 ){
			textExp += ' (' + logic + ')';
