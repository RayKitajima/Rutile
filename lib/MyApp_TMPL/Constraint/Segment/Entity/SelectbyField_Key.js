...
// Selectby{{#Uc_first}}{{field}}{{/Uc_first}}
// 
// segment : {{segment}}
// entity  : {{entity}}
// field   : {{field}}
...
//     var selector = new SelectbyField({ values : [1,2,3], logic : "and" });
//     
...
Selectby.makeConstraint = function(){
	...
	sql = " ( select {{#Escape_reserved}}{{#Lc_first}}{{entity}}{{/Lc_first}}{{/Escape_reserved}}.{{primary_key}} from {{#Escape_reserved}}{{#Lc_first}}{{entity}}{{/Lc_first}}{{/Escape_reserved}} where " + clause + " ) ";
...
Selectby.stringify = function(){
	var str = "{{#Lc_first}}{{entity}}{{/Lc_first}}.{{field}}({{search_type}}):" + this.values.join(",") + "/" + logic;
	return str;
};
