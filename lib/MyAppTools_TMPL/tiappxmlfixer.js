...
function run(locale) {
	...
	tiapp.modules.map( function(m){ installed_modules[m.id]=1; } );
	...
	if( !tiapp['ios'] ){
		...
		if( tiapp['ios']['plist'] ){
			tiapp['ios']['plist']['UIStatusBarStyle'] = "UIStatusBarStyleLightContent";
			tiapp['ios']['plist']['UIViewControllerBasedStatusBarAppearance'] = false;
		}else{
			tiapp['ios']['plist'] = {"UIStatusBarStyle":"UIStatusBarStyleLightContent","UIViewControllerBasedStatusBarAppearance":false};
		}
	}
